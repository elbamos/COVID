...
#Countries to fix:  Quebec, Inner Mongolia, Ningxia, Xinjiang, Tibet, Hong Kong
...
supplement_data_for_growth <- function(x) x %>% 
  mutate(
    ...
    new_cases_ma = rollmeanr(new_cases, k=5, fill=new_cases),
    growth_rate_ma = rollmeanr(growth_rate, k=5, fill=growth_rate),
    ...
    active_case_growth_rate_ma = rollmeanr(active_case_growth_rate, k=5, fill=active_case_growth_rate),
...
output$world_growth <- renderHighchart({
  ...
  highchart()  %>%
    ...
    hc_add_series(
      ...
      marker=list(enabled=F), 
      data = world_case_summary() %>% 
        select(date, `Growth Rate` = growth_rate, `Growth Rate 5-day Rolling Average`=growth_rate_ma) %>%
...
make_us_cases <- function(county_cases, state_cases, covid_tracker) geometries %>%
  ...
  mutate(
    label = paste0(
      ...
      "5-Day Trailing Average New Cases / Day: ", round(new_cases_ma), "<br/>", 
      "5-Day Trailing Average Growth / Day: ", round(growth_rate_ma * 100, 1), "%<br/>",
      if_else(is.na(county), paste0("5-Day Trailing Average Change in Active Cases / Day", round(active_case_growth_rate_ma * 100, 1), "%<br/>"), "")
    ),
    ...
    hc_add_series(
      ...
      data=us_case_summary() %>% 
        select(
          ...
          `Growth Rate 5-day Moving Average` = growth_rate_ma
        ) %>% 
        gather(key="key", value="value", -date) %>%
...
### Curve Flattening (# of cases vs. 5-day moving average new cases/day)
