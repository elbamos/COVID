...
world_vis_options <- reactive({
  minmax <- world_cases_jh() %>%
    ...
    filter(!is.na(val), !is.infinite(val), !is.nan(val),
           val > 0 | ! key %in% c("confirmed", "growth_rate_ma", "new_cases_ma")) %>%
    group_by(key) %>% 
    summarize(
      min=min(val), 
      max=max(val)
  ...
  list(
    "Growth Rate Moving Average" = list(
      ...
      legend_trans = labelFormat(transform=function(x) 10^x, suffix = " %")
    ),
    "Cases" = list(
      pallete = "inferno", 
    ...
    "Active Case Growth Rate Moving Average" = list(
      ...
      legend_bins=5, 
      legend_vals=ranges[["active_case_growth_rate_ma"]], 
      legend_trans = labelFormat(transform=round)
    )
  )
})
```
