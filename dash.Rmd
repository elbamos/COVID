...
```{r todods,include=F}
# TODO: US MAP
# Rate legend is wrong
# TODO: NY Chart needs popups
# TODO: Add to interventions, charts showing the first and second derivative
...
make_us_cases <- function(county_cases, state_cases)
  geometries %>%
  inner_join(
    county_cases %>%
      filter(cases > 0) %>%
  ...
  mutate(
    ...
    label = paste0(
      ...
      "<hr>",
      "Population: ", pop, "<br/>",
      "Density: ", pop_density, " ppl/sqmi<br/>",
      "Housing Units: ",  housing_density, " /sqmi<br/>",
      "<hr>",
      "Confirmed Cases: ", cases, "<br/>", "Infection Rate: ", rate, " / 100,000 ppl<br/>",
      "Deaths: ", deaths, "<br/>",
      "<hr>",
      "New Cases: ", new_cases, "<br/>",
      "Growth: ", growth_rate, " d/d<br/>",
      "Change in Growth: ", delta_growth_rate, "<br/>"
    ),
    label = map(label, HTML)
  )


us_map_filter <- reactive(make_us_cases(dataset$cases_county_nyt()))
```
...
us_cases <- reactive({
  req(input$usdate)
  us_map_filter() %>% filter(date == input$usdate, cases > 0) 
})
...
pal_rate <- colorNumeric("Oranges", domain = log10(c(1, 1e5)))
...
interventions_with_cases <- reactive({
  ...
  dataset$cases_county_nyt() %>%
    ...
    inner_join(locations) %>% 
    mutate(name = paste0(Subsublocation, ", ", Sublocation))
})
```
