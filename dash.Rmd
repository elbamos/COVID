...
zip_geoms %<>% rbind(ny_zip_geoms) %>% rbind(alameda)
...
process_nyzipcode <- function(x) x %>% 
  ...
  select(zipcode, cases=Positive, total_tests=Total, date) %>%
  mutate(recovered = NA, deaths=NA)
...
city_vis_options <- get_vis_elements(city_data_over_time,
                                     "city_vis_options")

city_data <- reactive({
  req(input$citydate)
  city_data_over_time() %>% 
    filter(date == max(date)) # input$citydate) 
})
```
...
output$cityview <- renderLeaflet({
  ...
  req(city_data_over_time)
  req(city_data)
  
  geoms <- city_data_over_time() %>%
    group_by(zipcode) %>%
    filter(row_number() == 1) %>%
    ungroup() %>% 
    use_series(geometry)
  ...
  bbox = st_bbox(geoms)
  centroid = st_bbox(geoms)
  
  vis_options <- isolate(city_vis_options())

  map <- leaflet(isolate(city_data())) %>% 
    addProviderTiles(providers$CartoDB.Positron) %>%
    addLayersControl(baseGroups = names(vis_options))  %>% 
   # setMaxBounds(bbox[[1]], bbox[[2]], bbox[[3]], bbox[[4]]) %>% 
    #setView(lat=centroid[[1]][2], lng=centroid[[1]][1], zoom=12) %>% 
...
observe(label="update_citymap", {
  ...
  leafletProxy(data=city_data(), "cityview", deferUntilFlush=T) %>% 
    update_map(vis_options, "cityview", input)
})
