...
rate_pal <- colorNumeric("Reds", domain=c(0, 1000))
```
...
output$sfview <- renderLeaflet({
  leaflet(sfzip_geoms() %>% filter(date == max(date))) %>% 
    ...
    #setView(lat=sf_centroid[[1]][2], lng=sf_centroid[[1]][1], zoom=10) %>%
    addLegend(pal=rate_pal, values=c(0, 50, 100, 1000), 
              title="Positive Rate")  %>%
    addPolygons(weight=0, opacity=.3, stroke=F, fill=T, 
               fillOpacity = 0.4, fillColor=~rate_pal(rate), 
               label=~label) 
})
```
