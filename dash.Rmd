...
```{r include=F,context="server-start",cache=F}
...
make_us_cases <- function(county_cases, state_cases, covid_tracker) geometries %>%
  ...
  mutate(
    ...
    label = paste0(
      ...
      "5-Day Trailing Average New Cases / Day: ", round(new_cases_ma, 3), "<br/>", 
      "5-Day Trailing Average Growth / Day: ", round(growth_rate_ma, 3), "<br/>"
    ),
...
vis_options <- reactive({
  ...
  out <- list(
    "Growth Rate Moving Average" = list(
      ...
      legend_bins = 5, 
      legend_vals = log10(100 * c(.01, 1.5)), 
      legend_trans = labelFormat(transform=function(x) 10^x, suffix = " % daily growth")
    ),
    "Infection Rate" = list(
      pallete = "inferno", 
    ...
    "New Cases Moving Average" = list(
      ...
      legend_bins=5, 
      legend_vals=log10(ranges[["new_cases_ma"]]), 
      legend_trans = labelFormat(transform=function(x) 10^x, suffix = " new cases per day")
    ),
    "Cases" = list(
      pallete = "Reds", 
  ...
  if (isolate(input$usscale) == "State") {
    out <- c(out, list(
             # "Hospitalization Rate" = list(
             #   pallete = "Spectral", 
             #   pal = quote(hosp_rate), 
             #   legend_bins=6, 
             #   lengend_vals = ranges[["hosp_rate"]], 
             #   legend_trans = labelFormat(transform=identity, suffix=" Hospitalizations / Case")
             # ), 
             "ICU Rate" = list(
               pallete = "YlGnBu", 
               pal = quote(icu_rate), 
