...
url_alameda <- "https://opendata.arcgis.com/datasets/e9ed9fa8d5d54d0e80dbfb95b2e142f5_0.csv" 

#"https://opendata.arcgis.com/datasets/e9ed9fa8d5d54d0e80dbfb95b2e142f5_0.geojson"

process_alameda <- function(x) x %>% 
  select(DtCreate, matches("F[0-9]+")) %>% 
  gather(key = "zipcode", value = "cases", -DtCreate) %>% 
  mutate(
    date = as.Date(ymd_hms(DtCreate)),
    zipcode = as.numeric(str_match(zipcode, "F([0-9]+)")[, 2]),
    cases = case_when(
      str_detect(cases, "<") ~ 5, 
      TRUE ~ as.numeric(cases)
    )
  ) %>% 
  group_by(zipcode) %>% filter(date == max(date)) %>% ungroup() %>% select(zipcode, cases)

dataset$alameda <- makePoll("Alameda", 
                              url_alameda, 
                              process_alameda)
```
...
get_vis_elements <- function(x, label) reactive(label=label, {
  ...
  list(
    ...
    "Pop Density" = list(
      ...
      legend_trans = labelFormat(transform=function(x) round(10^x), suffix = " ppl / sqmi")
    ),
    "Active Cases" = list(
      field = "active_cases",
...
world_click_name <- reactiveVal("Global")

set_world_click_name <- function(new_loc_id) {
  if (is.na(new_loc_id)) world_click_name("Global")
  else locations %>%
    filter(loc_id == new_loc_id) %>%
    mutate(
      name = case_when(
...
match_geoms <- function(click, geoms) {
  lat <- click$lat
  lng <- click$lng
...
world_click_state <- reactiveVal(NA)

observeEvent(input$worldview_click, label="worldmapclick", {
  {
    poly_match <- match_geoms(input$worldview_click, isolate(world_geoms))
    
    print("got geoms")
    
    if (nrow(poly_match) > 0) {
      out <- poly_match$loc_id[1]
      set_world_click_name(out)
      out
    } else {
      circle_matches <- match_circles(input$worldview_click)
      
      if (circle_matches$d[1] < click_d_threshold) {
        set_world_click_name(circle_matches$loc_id[1])
      } else {
        set_world_click_name(NA)
      }
    }
  } %>% world_click_state()
})
...
output$worldcases <- renderHighchart({
  ...
  highchart()  %>%
    hc_title(text = paste0("Cases (", world_click_name(), ")")) %>%
    hc_yAxis(title = "", 
...
output$dailynewcases <- renderHighchart({
  req(input$worldscale)
  highchart()  %>%
    hc_title(text = paste0("New Cases Per Day (", world_click_name(), ")")) %>%
    hc_yAxis(title = "") %>%
    hc_xAxis(type = "datetime") %>%
    hc_add_series(
      type = "column",
...
output$world_growth <- renderHighchart({
  req(input$worldscale)
  highchart()  %>%
    hc_title(text = paste0("Growth Rate (", world_click_name(), ")")) %>%
    hc_yAxis(title = "", 
             type="logarithmic",
             labels=list(
               format="{value} %"
...
region_click_state <- reactiveVal(NA)

observeEvent(input$usview_click, label="usmapclick", {
  out <- match_geoms(input$usview_click, req(us_cases_with_geo())) %>%
    select(state, county, loc_id)
  
  if (nrow(out) == 0) {
    click_state_name("U.S.")
    region_click_state(NA)
  } else {
    name <- if_else(is.na(out$county[1]), out$state[1],
                    paste0(out$county[1], ", ", out$state[1]))
    click_state_name(name)
    region_click_state(out)
  }
})
```
...
output$uscaseslinear <- renderHighchart(
  highchart() %>% 
    hc_title(text=paste0("Cumulative (", click_state_name(), ")")) %>%
    ...
    hc_add_series(
      ...
      data=req(us_case_summary()) %>% 
        select(date, Active, Cases, Deaths, Hospitalized, ICU, Ventilator, Recovered) %>% 
        gather(key="Series", value="val", -date) %>%
        filter(val > 0), 
      mapping=hcaes(x=date, y=val, group=Series)
    ) %>%
...
output$usdailynewcases <- renderHighchart({
  highchart() %>% 
    hc_title(text=paste0("Daily New (", click_state_name(), ")")) %>%
    hc_xAxis(type="datetime") %>%
...
output$us_growth_rate <- renderHighchart(
  highchart() %>% 
    hc_title(text=paste0("Growth Rate (", req(click_state_name()), ")")) %>%
    hc_xAxis(type="datetime") %>%
...
output$ustesting <- renderHighchart(
  highchart()  %>%
    ...
    hc_title(text=paste0("Cumulative Testing (", click_state_name(), ")")) %>%
...
output$ustestinginc <- renderHighchart(
  highchart()  %>%
    hc_plotOptions(column=list(stacking="normal")) %>%
    hc_title(text=paste0("Daily Testing (", click_state_name(), ")")) %>%
    hc_yAxis(title="", min=0) %>%
    hc_xAxis(type="datetime") %>% 
    hc_add_series(data = req(us_case_summary()) %>% 
...
output$ustestingrate <- renderHighchart(
  highchart()  %>%
    hc_plotOptions(column=list(stacking="normal")) %>%
    hc_title(text=paste0("Positive Rate (", click_state_name(), ")")) %>%
    hc_yAxis(title="", min=0) %>%
    hc_xAxis(type="datetime") %>% 
    hc_add_series(data = req(us_case_summary()) %>% 
