...
update_map <- function(data, vis_options, map_name) {
  map <- leafletProxy(data=data, map_name, deferUntilFlush=T) %>%
...
observe(label="update_world_map", {
  ...
  map <- update_map(polys, vis_options, "worldview")
  ...
  if (! is.null(groups)) {
    map %>% clearControls() 
    adjust_legend(map, groups, vis_options)
  }
})
...
observe(label="update_usmap", {
  ...
  map <- update_map(us_cases_date_filtered(), vis_options, "usview")

  map %<>%
      addLayersControl(baseGroups = names(vis_options))
  
  groups <- isolate(input$usview_groups) 
  if (! is.null(groups)) adjust_legend(map, groups, vis_options)
})
...
```{r sfmap_data,context="server-start",include=F}
sf_map_data <- reactive(sfzip_geoms() %>% filter(date == max(date)))

sf_vis_options <- get_vis_elements(sf_map_data, "sf_vis_options")
```
