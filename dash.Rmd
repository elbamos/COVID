...
process_world_jh <- function(confirmed_in, dead_in, recoveries_in) {
  ...
  full_join(confirmed_by_date, dead) %>% full_join(recoveries) %>%
    group_by(loc_id) %>% 
...
recoveries <- read_csv("./COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv")

world_cases_jh <- process_world_jh(confirmed, dead, recoveries)
...
dataset$world_cases_jh <- reactivePoll(
  intervalMillis = updateFreq, session=NULL,
  checkFunc = function() min(get_url_date(url_world_confirmed),
                             get_url_date(url_world_dead), 
                             get_url_date(url_world_recovered)), 
  valueFunc = function() {
    confirmed <- read_csv(url_world_confirmed)
    dead <- read_csv(url_world_dead) 
    recoveries <- read_csv(url_world_recovered)
    process_world_jh(confirmed, dead, recoveries)
  }
)
...
output$datetoshow <- renderText(input$usdate)
