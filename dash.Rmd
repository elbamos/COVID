...
sub_countries <- chn %>% # rbind(aus, can, chn) %>%
  mutate(pop_est = NA)
...
#Countries to fix:  Australia, Canada, Tanzania, Yemen, certain Chinese provines 
...
world_cases <- reactive({
  ...
  todisplay %>% 
    ...
    select(
      ...
      confirmed, dead, new_dead, new_confirmed, new_cases_ma, 
...
world_vis_options <- reactive({
  minmax <- world_cases_jh() %>%
    ungroup() %>%
    select(confirmed, dead, recovered, growth_rate, new_cases_ma, growth_rate_ma) %>% 
...
output$worldview <- renderLeaflet({
  leaflet(
    ...
    addLayersControl(baseGroups=c("Cases", "Growth Rate Moving Average", "New Cases Moving Average"))
})
