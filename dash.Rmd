...
reactivePoll(
  intervalMillis = updateFreq, session=NULL,
  checkFunc = function() get_url_date(url_sfzipcode),
  valueFunc = function() get_new_data_json(tag="SF Testing by Zipcode", 
                                           url=url_sfzipcode, 
                                           f=process_sfzipcode)
)
```
...
```{r palettes,context="server-start",cache=F,include=F}
palletes <- list(
  # Counts
  confirmed = "Reds", 
  cases = "Reds", 
...
```{r phony_inputs,eval=F,include=F}
...
tagList(
  ...
  h3("New York City Testing"),
  p("The NYC Testing pane shows the number of positive tests, and proportion of tests that are positive, by New York City zipcode."),
  p("This view can be used to explore how socioeconomic factors influence both test availability and susceptibility to the virus."), 
  h3("Interventions"), 
  p("The interventions pane shows the effect of different interventions on the rate of infection."),
  p("Each line represents a county. The x axis is the number of days from the time that county hit a threshold number of infections."),
  p("The points on the lines show the dates on which different interventions were imposed."),
  h3("Curve Flattening"),
...
output$status_nydoh <- data_status_box(dataset$nyzipcode, "NY Testing by Zipcode", "NY DOH")
```
...
world_cases <- reactive({
  ...
  todisplay %>% 
    ...
    select(
      ...
      cases_growth_rate, cases_growth_rate_ma,
      active_cases, active_case_growth, active_case_growth_rate, active_case_growth_rate_ma,
      hot_spot
      ) %>% 
    mutate(
      popup=paste0('<font color="blue">', 
                   ...
                   "<br/>Case Growth Rate Moving Average: ", round(cases_growth_rate_ma * 100), "% change in newly diagnosed cases per day",
...
world_vis_options <- reactive({
  minmax <- world_cases_jh() %>%
    ungroup() %>%
    select(confirmed, dead, recovered, cases_growth_rate, new_cases_ma, cases_growth_rate_ma, active_case_growth_rate_ma) %>% 
    gather(key="key", value="val") %>%
    filter(!is.na(val), !is.infinite(val), !is.nan(val),
           if_else(key %in% c("confirmed", "cases_growth_rate_ma", "new_cases_ma"), val > 0, TRUE)) %>%
    group_by(key) %>% 
    summarize(
      min=min(val), 
      max=max(val)
  ...
  list(
    ...
    "Growth Rate Moving Average" = list(
      pallete = palletes$cases_growth_rate_ma,
...
make_us_cases <- function(county_cases, state_cases, covid_tracker) geometries %>%
  ...
  mutate(
    label = paste0(
      ...
      "<hr>",
      if_else(is.na(county), paste0("Active Cases: ", active_cases, "<br/>"), ""),
      ...
      if_else(is.na(county),
              ...
              , " "),
      "<hr>",
      "New Cases: ", cases_new, "<br/>",
      "Growth: ", round(cases_growth_rate, 3), " d/d<br/>",
      params$k, "-Day Trailing Average New Cases / Day: ", round(new_cases_ma), "<br/>", 
      params$k, "-Day Trailing Average Growth / Day: ", round(cases_growth_rate_ma * 100, 1), "%<br/>",
...
vis_options <- reactive({
  minmax <- us_cases_state_county() %>%
    ...
    select(pop, cases, deaths, negative, pending, hospitalized, icu, vent, recovered, 
           pop_density, housing_density, occupancy_density, rate, pos_rate, cases_new, 
           cases_growth_rate, penetrance, hosp_rate, icu_rate, vent_rate,
           new_cases_ma, cases_growth_rate_ma, active_cases, active_case_growth_rate_ma) %>% 
    gather(key="key", value="val") %>%
    filter(!is.na(val), !is.infinite(val), !is.nan(val),
           val > 0 | ! key %in% c("pop_density", "housing_density", "rate", "cases", 
                                  "new_cases_ma", "cases_growth_rate_ma", 
                                  "active_cases")) %>%
    group_by(key) %>% 
    summarize(
      min=min(val), 
  ...
  if (isolate(input$usscale) == "State") {
    out <- c(out, list(
            "Active Cases" = list(
              pallete = palletes$active_cases,
              pal = quote(log10(active_cases)),
              legend_bins=3,
              legend_vals=log10(ranges[["active_cases"]]),
              legend_trans=labelFormat(transform=function(x) round(10^x))
            ),
            "Active Case Growth Rate Moving Average" = list(
              pallete = palletes$active_case_growth_rate_ma, 
...
ny_bbox <- st_bbox(zips %>% filter(! str_starts(zipcode, "94")) %>% use_series(geometry))
ny_centroid <- st_centroid(zips %>% filter(! str_starts(zipcode, "94")) %>% use_series(geometry))
...
output$nyview <- renderLeaflet({
  leaflet(nyzip_geoms()) %>% 
    ...
    addLegend(pal=rate_pal, values=c(0, .25, .5, .75, 1), 
              title="Positive Rate")  %>%
    addPolygons(weight=1, opacity=.3, stroke=T, fill=T, 
                color="blue",
...
```{r sfmap_startup,context="server-start",include=F}
sfzip_geoms <- reactive(
  zip_geoms %>% 
    left_join(zips %>% data.frame() %>% select(zipcode, aland10)) %>%
    inner_join(dataset$sfzipcode() %>% mutate(zipcode = as.character(zipcode))) %>% 
    mutate(
      rate = (cases / pop) * 100000,
      label = paste0(
        "<font color='blue'>Zipcode: ", zipcode, "</font><br/>",
        "Cases: ", cases, " / ", pop, 
        "<br/>Infection rate: ", round(rate), " / 100,000ppl"
      ),
      label = map(label, HTML)
    )
)

sf_bbox <- st_bbox(zips %>% filter(str_starts(zipcode, "94")) %>% use_series(geometry))
sf_centroid <- st_centroid(zips %>% filter(str_starts(zipcode, "94")) %>% use_series(geometry))
rate_pal <- colorNumeric(palletes$rate, domain=log10(c(1, 1000)))
cases_pal <- colorNumeric(palletes$cases, domain=log10(c(1, 1000)))
```

```{r sfmap_server,context="server",include=F}
output$sfview <- renderLeaflet({
  leaflet(sfzip_geoms() %>% filter(date == max(date))) %>% 
    addProviderTiles(providers$CartoDB.Positron) %>% 
    setMaxBounds(sf_bbox[1], sf_bbox[2], sf_bbox[3], sf_bbox[4]) %>%
    addPolygons(weight=.1, opacity=.3, stroke=T, fill=T, 
                group="Infection Rate", 
               fillOpacity = 0.4, fillColor=~rate_pal(log10(rate)), 
               label=~label,
               highlight = highlightOptions(
            weight=5, fillOpacity=1.0, bringToFront = TRUE
          )) %>%
    addPolygons(weight=.1, opacity=.3, stroke=T, fill=T, 
                group="Cases",
           fillOpacity = 0.4, fillColor=~cases_pal(log10(cases)), 
           label=~label,
            highlight = highlightOptions(
            weight=5, fillOpacity=1.0, bringToFront = TRUE
          )) %>%
    addLayersControl(baseGroups=c("Infection Rate", "Cases"))
})
```

```{r sfmap_output}
leafletOutput("sfview")
```
...
output$interventions_rate <- renderHighchart({
  highchart() %>% 
    hc_add_series(data=filtered_interventions_with_cases(), type="line", 
                  mapping=hcaes(x=days, y=cases_growth_rate, group=name), 
                  colorAxis=0,
                  lineWidth=1, 
                  marker=list(enabled=F),
                  tooltip=list(
                    pointFormat="Date: {point.date}<br/>Cases: {point.cases}<br/>Growth Rate: {point.cases_growth_rate}"
                  )) %>% 
    hc_yAxis(title="Growth Rate", type="logarithmic") %>%
    hc_add_series(data=intervention_points(), type="point", 
                  mapping=hcaes(x=days, y=cases_growth_rate, fill=name,
                                color=name, group=intervention), 
                  colorAxis=0, tooltip=list(
                    headerFormat="<span style='color:{point.color}'>●</span> <span style='font-size: 12px'> {series.name}</span><br/>",
                    pointFormat="{point.name}<br/>Date: {point.date}<br/>Cases: {point.cases}<br/>Growth Rate: {point.cases_growth_rate}"
                  )) %>%
...
output$curve_flattening_inputs <- renderUI({
  tagList(
    radioButtons("curve_flattening_resolution", label="Resolution", choices=c("Countries", "States", "Counties"), selected="Countries"),
    sliderTextInput(
