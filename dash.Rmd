...
```{r todods,include=F}
# TODO: Mapping
#      - Make a frame containing country maps
#      - Add regional maps for Australia, Canada, and China
#      - Split the world_jh into one set that can link onto the regional maps, display that as polygons
#      - Continue to display the non-matching world_jh as circles
...
world_cases <- reactive({
  ...
  todisplay %>% 
    ...
    ungroup() %>% 
    select(confirmed, dead, lat, lng, Sublocation, Subsublocation, Location, loc_id) %>% 
    mutate(
...
output$worldview <- renderLeaflet({
  leaflet(
    ...
    addLayersControl(overlayGroups=c("Confirmed", "Deaths", "Growth Rate"))
})
...
observe(label="update_world_map", {
  ...
  cases_pal <- colorNumeric("Oranges", domain=log10(c(1, max(polys$confirmed))))
  deaths_pal <- colorNumeric("Reds", domain=log10(c(1, max(polys$dead))))
  growth_rate_ma_pal <- colorNumeric("Blues", domain=log10(c(min(polys$growth_rate_ma), max(polys$growth_rate_ma))))
    
  leafletProxy(data=polys, "worldview", deferUntilFlush=T) %>%
    clearShapes() %>% clearMarkers() %>% 
    addPolygons(
      group="Confirmed", 
      fillColor = ~cases_pal(log10(confirmed)),
      fillOpacity = .7,
      label=~popup
    ) %>%
    addPolygons(
      group="Deaths", 
      fillColor = ~cases_pal(log10(dead)),
      fillOpacity = 0.7,
      label=~popup
    ) %>%
    addPolygons(
      group="Growth Rate", 
      fillColor = ~ growth_rate_ma_pal(log10(growth_rate_ma)),
      fillOpacity = 0.7,
      label=~popup
    ) %>%
