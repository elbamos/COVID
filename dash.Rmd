...
supplement_testing_data <- function(x) x %>% 
  mutate(
...
us_case_summary <- reactive(label="us_case_summary", {
  cases <- us_cases_state_county() %>% ungroup()
  ...
  cases %>% 
    select(date, Cases=cases, Deaths=deaths, 
           ...
           cum_negative, pending, negativeIncrease, cum_pos_rate, 
...
output$ustestingrate <- renderHighchart(
  highchart()  %>%
    ...
    hc_add_series(data = us_case_summary() %>% 
                    ...
                    select(date, `Positive Rate`= cum_pos_rate), 
                  type="line", 
                  name="Positive Rate",
                  mapping=hcaes(x=date, y=`Positive Rate`))
)
```
