...
```{r todods,include=F}
# TODO: Need to change interventions popup
# TODO: Switch interventions between log and flat view
...
interventions <- read_csv("covid19-intervention-data/npi_data_03-24-2020.csv") %>% 
  ...
  mutate(
    ...
    county = if_else(
      is.na(county) & str_detect(Intervention, "DC"), "Washington", county
    ), 
    state.abb = if_else(is.na(state.abb), "DC", state.abb)
  ) %>% 
  left_join(data.frame(Intervention = rep("New York City, NY", 5), nycounty = c(
    "New York", "Kings", "Queens", "Bronx", "Staten Island"
  ))) %>% 
  mutate(county = if_else(!is.na(nycounty), as.character(nycounty), county)) %>% 
  select(-nycounty) %>% 
  select(-Intervention) %>% 
...
output$uscaseslog <- renderHighchart(
  us_case_summary %>% select(date, Confirmed=cases, Deaths=deaths) %>%
    gather(key="Series", value="val", -date) %>%
    hchart("line", hcaes(x=date, y=val, group=Series)) %>% 
    hc_yAxis(type = "logarithmic", gridLineWidth = 0, title="", min=1) %>%
...
output$ustesting <- renderHighchart(
  covid_track %>% 
    ...
    hchart("column", hcaes(x=date, y=val, group=Series, fill=Series)) %>%
    hc_plotOptions(column=list(stacking="normal")) %>%
    hc_title(text="Progress of US Testing") %>%
    hc_yAxis(title="")
)
...
output$ustestinginc <- renderHighchart(
  covid_track %>% 
    ...
    summarize_all(~sum(., na.rm=T)) %>%
    arrange(date) %>% 
    mutate_if(is.numeric, ~ . -  lag(., 1, default=0)) %>% 
    gather(key="Series", val="val", -date) %>% 
    filter(Series != "pending") %>% 
    hchart("column", hcaes(x=date, y=val, group=Series, fill=Series)) %>%
    hc_plotOptions(column=list(stacking="normal")) %>%
    hc_title(text="Daily New Test Results") %>%
    hc_yAxis(title="")
)
...
counties_with_intervention <- cases_county_nyt %>% semi_join(locs_with_intervention) 

interventions_with_cases <- interventions %>% 
...
avail_states <- counties_with_intervention %>% 
  inner_join(locations) %>%
...
output$interventions <- renderHighchart(
  highchart() %>% 
    hc_add_series(data=counties_to_show(), type="line", 
                     mapping=hcaes(x=date, y=cases, group=name), 
                     colorAxis=0,
                     lineWidth=1, marker=list(enabled=FALSE)) %>% 
    hc_yAxis(title="Cases", type="logarithmic", min=1) %>%
    hc_xAxis(type="datetime") %>%
    hc_add_series(data=interventions_with_cases(), type="point", 
                  mapping=hcaes(x=date, y=cases, fill=name,
