...
us_case_summary <- reactive(label="us_case_summary", {
  ...
  cases %>% 
    select(date, Cases=cases, Deaths=deaths, 
           ...
           cum_negative, pending, negativeIncrease,
...
output$ustesting <- renderHighchart(
  highchart()  %>%
    ...
    hc_title(text="Cumulative Testing") %>%
    hc_yAxis(title="") %>%
    hc_xAxis(type="datetime") %>% 
...
output$ustestinginc <- renderHighchart(
  highchart()  %>%
    hc_plotOptions(column=list(stacking="normal")) %>%
    hc_title(text="Incremental Testing") %>%
    hc_yAxis(title="") %>%
    hc_xAxis(type="datetime") %>% 
    hc_add_series(data = us_case_summary() %>% 
...
tabsetPanel(type="tabs", 
            ...
            tabPanel("Testing", 
                     fluidRow(
                       column(6, highchartOutput("ustesting")),
                       column(5, highchartOutput("ustestinginc"), 1), 
