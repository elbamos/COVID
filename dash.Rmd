...
world_cases_jh <- reactive({
  full_join(dataset$confirmed(), dataset$dead()) %>%
    ...
    mutate(
      ...
      pop = NA, # Find pop later for more data!
    ) %>% 
...
process_nyzipcode <- function(x) x %>% 
  ...
  select(zipcode, Positive, Total, date) 

get_one_ny_day <- function(commit, date) read_csv(
  paste0(
...
scrape_ny <- function() {
  commits <- read_html("https://github.com/nychealth/coronavirus-data/commits/master/tests-by-zcta.csv")
  ...
  commits %>% 
    ...
    bind_cols(dates) %>% 
    mutate(
      data = map2(commit, date, ~get_one_ny_day(.x, .y))
    ) %>%
    use_series(data) %>%
    bind_rows() 
}

get_ny_data <- function(tag, f) {
  out <- scrape_ny() %>% 
    f()
  update_dataset_dt_url(tag, url_nyzipcode)
  out 
}
...
dataset$nyzipcode <- reactivePoll(
  ...
  valueFunc = function() get_ny_data("NY Testing by Zipcode", process_nyzipcode)
)
```
...
observe(label="update_usmap", {
  ...
  map <- leafletProxy(data=us_cases_date_filtered(), "usview", deferUntilFlush=T) %>%
...
sfzip_geoms <- reactive(
  zip_geoms %>% 
    ...
    mutate(
      ...
      popup = paste0(
        "<font color='blue'>Zipcode: ", zipcode, "</font><br/>",
        "Cases: ", cases, " / ", pop, " population<br/>", 
        "Infection rate: ", round(rate), " / 100,000 ppl<br/>",
        "Density: ", round(pop_density), " ppl / sqmi<br/>",
        "<hr>",
        "New Cases: ", cases_new, "<br/>",
        "Growth: ", round(cases_growth_rate, 3), " d/d<br/>",
        params$k, "-Day Trailing Average New Cases / Day: ", round(new_cases_ma), "<br/>", 
...
```{r sfmap_data,context="server",include=F}
sf_map_data <- reactive(sfzip_geoms() %>% filter(date == max(date)))

sf_vis_options <- get_vis_elements(sfzip_geoms, "sf_vis_options")
```


```{r sfmap_map,context="server",include=F}
output$sfview <- renderLeaflet({
  req(sf_map_data)
  req(sf_vis_options)
  
  map_data <- isolate(sf_map_data())
  vis_options <- isolate(sf_vis_options())
  
  sf_bbox <- map_data %>% use_series(geometry) %>% st_bbox()
  sf_center <- map_data %>% use_series(geometry) %>% st_centroid()
  
  map <- leaflet(map_data) %>% 
    addProviderTiles(providers$CartoDB.Positron) %>%
    addLayersControl(baseGroups = names(vis_options))  %>% 
    setMaxBounds(sf_bbox[[1]], sf_bbox[[2]], sf_bbox[[3]], sf_bbox[[4]]) %>% 
    setView(lat=sf_center[[1]][2], lng=sf_center[[1]][1], zoom=12) %>% 
    update_map(vis_options, "sfview", input)
})

observe(label="update_sfmap", {
  req(sf_vis_options)
  req(sf_map_data)
  vis_options <- sf_vis_options()
  
  leafletProxy(data=sf_map_data(), "worldview", deferUntilFlush=T) %>% 
    update_map(vis_options, "sfview", input)
})

observeEvent(input$sfview_groups, 
             label="handle_sfmap_legend", {
  
  req(sf_vis_options)
  map <- leafletProxy('sfview') %>% 
    clearControls() 
  
  selection <- isolate(input$sfview_groups)
  
  adjust_legend(map, selection, sf_vis_options())
})
```
