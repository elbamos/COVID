...
world_case_summary <- reactive(label="world_case_summary", {
  ...
  click_state <- if (isTruthy(world_click_state())) world_click_state() 
  else NA_real_
...
us_case_summary <- reactive(label="us_case_summary", {
  click_state <- if (isTruthy(region_click_state())) region_click_state()
  else NA
...
output$uscaseslinear <- renderHighchart(
  highchart() %>% 
    hc_title(text=paste0("Cases, Hosp. and Deaths (", click_state_name(), ")")) %>%
...
output$usdailynewcases <- renderHighchart({
  highchart() %>% 
    hc_title(text=paste0("Daily New Cases (", click_state_name(), ")")) %>%
    hc_xAxis(type="datetime") %>%
