...
process_interventions <- function(x) x %>%
  ...
  select(-matches("state"), -county) %>% glimpse()
...
```{r pull_census, include=F,eval=F}
library(tidycensus)
...
us_map_filter <- reactive(geometries %>% 
  ...
  left_join(
    ...
    mutate(
      ...
      housing_density = housing_units / aland, 
...
pal_density_pop <- colorNumeric("RdYlBu", domain = log10(c(1e-9, 1e-3)))
pal_density_hous <- colorNumeric("Greens", domain = log10(c(1, 1e-3)))
pal_density_occ <- colorNumeric("Blues", domain = c(0, 6))
pal_rate <- colorNumeric("Reds", domain = log10(c(1, 1000)))
...
vis_options <- list(
  ...
  "Occupancy Density" = list(
    pal = quote(~pal_density_occ(occupancy_density)), 
    legend_pal = pal_density_occ, 
    legend_vals = 0:6,
    legend_trans = identity
  ), 
  "Infection Rate" = list(    
    pal = quote(~pal_rate(rate)), 
    legend_pal = pal_rate, 
    legend_vals = log10(10^(0:4)),
    legend_trans = labelFormat(transform=function(x) 10^x)
    ), 
  "Cases" = list(
    pal = quote(~pal_cases(cases)), 
    legend_pal = pal_cases, 
    legend_vals = log10(10^(0:6)),
    legend_trans = labelFormat(transform=function(x) 10^x)
  )
)
...
observe({
  ...
  if (selection != "Cases") {
    map %>% addCircles(group="Confirmed Cases",
      ...
      addLayersControl(baseGroups=selection, overlayGroups="Confirmed Cases")
  }
})
